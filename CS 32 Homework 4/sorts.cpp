...
int main()
{
	...
	// time.
	{
		...
		for (size_t i = 0; i < auxStuds.size(); i++){
			Student* ptr = &(auxStuds[i]);
			studPtr.push_back(ptr);
		}
		...
		for (size_t i = 0; i < studPtr.size(); i++){
			studs.push_back(*(studPtr[i]));
		}
